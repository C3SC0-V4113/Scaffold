...
- The same file's `actions`, `thirdPartyActions`, and `toolchain` keys pin what generated workflows reference. `actions` holds GitHub-owned actions on version tags; `thirdPartyActions` holds everyone else's at a commit SHA with the version alongside, the same split `tests/workflow-pinning.test.ts` enforces on this repository's own workflows. `toolchain.pnpm` must stay an exact `x.y.z`: it is the fallback for a generated app's `packageManager` field, which rejects ranges. Never inline an action ref in `src/templates/files.ts`: an inlined ref is invisible to Renovate, and every app scaffolded before the next major bump keeps running a deprecated runner. That is exactly how generated pipelines ended up warning about node20. `tests/workflows.test.ts` and the E2E harness both fail if a generated workflow references an action that is not registered in `versions.json`, or pins a third-party action to anything other than a SHA plus version comment.
