...
export function renderShadcnComponentBoundariesSkill() {
  return `---
...
3. Use Purrfold-installed shared workflow skills when present; rerun \`sh skills.sh\` if they are missing.
...
3. Use Purrfold-installed supported Next.js workflow skills from \`vercel/next.js\` in \`.agents/skills/\` for Cache Components and dev-loop work when present; rerun \`sh skills.sh\` if they are missing.
4. Respect \`DESIGN.md\` for visual and UX decisions.
5. Use shadcn primitives and semantic tokens before custom styling.
6. Add tests proportional to risk.
